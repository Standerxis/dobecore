...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    -- Se o jogador estiver escrevendo no chat, ignore (gameProcessed)
    if gameProcessed then return end
    
    -- Verifica se a tecla apertada é a tecla configurada (ex: RightControl)
...
SettingsTab:Toggle("Esconder Minha Tag", false, function(state)
   ...
   -- Define um valor inicial caso não exista
Library.ToggleKey = Library.ToggleKey or Enum.KeyCode.RightControl

SettingsTab:Section("Atalhos")

SettingsTab:Button("Configurar Tecla", function()
    -- Este botão abre a espera pela tecla na sua Library
end, {
    Keybind = {
        Value = Library.ToggleKey,
        Callback = function(NewKey)
            Library.ToggleKey = NewKey
            print("Nova tecla definida para: " .. NewKey.Name)
        end
    }
})
