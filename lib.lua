...
function Library:UpdateKeybindRender()
    if self.KeybindListFrame then
        ...
        for _, item in pairs(self.KeybindListFrame:GetChildren()) do
            if item:IsA("Frame") then item:Destroy() end
        end
        
        for flag, keyCodeName in pairs(self.Flags.Binds) do
            -- Aqui você criaria os labels para mostrar na tela: "Aimbot: [F]"
            print("Atualizando render do bind: " .. flag .. " para " .. keyCodeName)
        end
    end
end
...
-- ==========================================================
local function AddExtras(parent, options, callback, manualFlag)
    if not options then return end
    
    -- 1. Definição da Flag (Identificador)
    local flag = manualFlag or (options and options.Flag) or "Unknown"
    if flag == "Unknown" then
        if parent:FindFirstChild("TextLabel") then
    ...
    if options.Keybind ~= nil then
        ...
        -- CARREGAMENTO INICIAL: Sincroniza valor salvo com a variável local
        local savedBind = _Config.Binds[flag] or Library.Flags.Binds[flag]
        if savedBind then
            ...
            if success then
                ...
                Library.Flags.Binds[flag] = savedBind
                Library:UpdateKeybindRender()
            end
        end

        -- Criação da UI (Com Estilo UX)
        ...
        -- Lógica de Animação e Reset Visual
        local function animateBox(show)
            TweenService:Create(KeyBtn, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { 
        ...
        -- FUNÇÃO ESSENCIAL: Permite que o botão "Carregar Config" mude a tecla externamente
        local function SetKey(newKeyName)
            local success, keyCode = pcall(function() return Enum.KeyCode[newKeyName] end)
            if success then
                if type(options.Keybind) == "table" then options.Keybind.Value = keyCode else options.Keybind = keyCode end
                KeyBtn.Text = keyCode.Name
                Library.Flags.Binds[flag] = keyCode.Name
                _Config.Binds[flag] = keyCode.Name
        ...
        Library.Elements[flag].SetKey = SetKey

        -- Interação de Clique (Binding)
        KeyBtn.MouseButton1Click:Connect(function()
            ...
            conInput = UserInputService.InputBegan:Connect(function(input)
                ...
                if isKeyboard or isMouse then
                    local selectedKey = (isKeyboard) and input.KeyCode or Enum.KeyCode.Unknown
                    if selectedKey == Enum.KeyCode.Escape then selectedKey = Enum.KeyCode.Unknown end
                    
                    SetKey(selectedKey.Name) -- Atualiza tudo via SetKey
        ...
        -- Listener do Gatilho da Tecla
        UserInputService.InputBegan:Connect(function(input, gp)
            if gp or binding then return end
            local currentKey = (type(options.Keybind) == "table" and options.Keybind.Value) or options.Keybind
            if currentKey == Enum.KeyCode.Unknown or currentKey == nil then return end
            
            if input.KeyCode == currentKey and blockKeyUntilRelease ~= currentKey then
    ...
    -- --- 2. SEÇÃO DE COLOR PICKER ---
    if options.Color then
        local savedColor = _Config.Colors[flag] or Library.Flags.Colors[flag]
        ...
        -- Função para carregar externamente
        ...
        ColorInd.MouseButton1Click:Connect(function()
            Library:OpenColorPicker(ColorInd.BackgroundColor3, function(newC)
                Library.Elements[flag].SetColor(newC.R, newC.G, newC.B)
                if SaveSettings then SaveSettings() end
            end)
        end)
...
function TabFuncs:Input(text, placeholder, callback)
    ...
    local function CreateOption(v)
        ...
        Option.Size = UDim2.new(1, -5, 0, 32)
    ...
    -- Lógica de fechar ao clicar fora (Compatível com Mobile e PC)
    UserInputService.InputBegan:Connect(function(input)
        local isClick = (input.UserInputType == Enum.UserInputType.MouseButton1)
        local isTouch = (input.UserInputType == Enum.UserInputType.Touch)

        if (isClick or isTouch) and dropped then
            task.wait(0.05) 
            -- Pega a posição baseado no tipo de input (Mouse ou Toque)
            ...
            -- Checagem de área (Adicionado pequena margem para toque se necessário)
            -- Nota: Mantive os 36px de compensação da TopBar conforme seu original
...
function Library:CreateSettings(Window)
    ...
    SettingsTab:Button("Importar Configuração", function()
        if ConfigInput == "" then 
            return NotificationService:Create("Erro", "Cole um código JSON válido no campo acima!") 
        end
        ...
        if success and decoded then
            ...
            NotificationService:Create("Import", "✅ Configuração importada com sucesso!")
            task.delay(1.2, function() NotificationService:Remove("Import") end)
        else
            NotificationService:Create("Erro", "❌ Código inválido ou corrompido.")
        end
    end)
    ...
    SettingsTab:Button("Exportar para Clipboard", function()
        ...
        if success then
            if setclipboard then
                setclipboard(json)
                NotificationService:Create("Export", "📋 JSON copiado para a área de transferência!")
            else
                print(json)
                NotificationService:Create("Export", "Aviso: JSON impresso no F9.")
            end
