...
-- Referências de movimento para Mobile
local UserInputService = game:GetService("UserInputService")
local RunService = game:GetService("RunService")
local Camera = workspace.CurrentCamera

-- =========================
-- MOVIMENTO (BASEADO NO SEU EXEMPLO)
-- =========================
RunService.RenderStepped:Connect(function()
    ...
    -- Pega a direção do Joystick/Analógico do Mobile
    local moveDirection = Hum.MoveDirection 
    
    -- Lógica de Subida e Descida (Baseado em InputFlags de sistema)
    local verticalMove = Vector3.zero
    
    -- No Mobile: Pulo faz subir
    if UserInputService:IsKeyDown(Enum.KeyCode.Space) or UserInputService.JumpHeld then
        verticalMove = Vector3.new(0, 1, 0)
    end
    
    -- No Mobile: Se o jogo tiver botão de agachar (C) ou se quiser usar lógica de CFrame
    -- Se quiser descer ao olhar para baixo e ir para frente, o MoveDirection já cuida disso.

    -- Cálculo final da Velocidade (Similar ao seu exemplo de flags)
    -- Multiplicamos o MoveDirection pela velocidade
    local finalVelocity = moveDirection * SPEED
    
    -- Adicionamos o movimento vertical
    BV.Velocity = finalVelocity + (verticalMove * VERTICAL_SPEED)
...
-- UI TOGGLE
-- =========================
ExploitTab:Toggle("Fly Mobile", false, function(state)
    if state then
        startFly()
    else
        stopFly()
