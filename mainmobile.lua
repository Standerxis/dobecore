...
-- =========================
-- LÓGICA DE MOVIMENTO MOBILE
-- =========================
RunService.RenderStepped:Connect(function()
    if not flying or not HRP or not Hum then return end

    -- Direção baseada no Joystick do Mobile
    local moveDirection = Hum.MoveDirection 
    
    -- Lógica de Subida/Descida (Botão de Pulo do Mobile)
    local verticalMove = 0
    if game:GetService("UserInputService").JumpHeld then
        verticalMove = 1
    end

    -- Aplica a velocidade baseada para onde o jogador aponta o analógico
    -- e para onde a câmera está olhando
    BV.Velocity = (moveDirection * SPEED) + Vector3.new(0, verticalMove * VERTICAL_SPEED, 0)
