...
-- =========================
local AjusteX = 25 -- Seu ajuste que funcionou
local AjusteY = 0  
...
local function GetCenter(inst)
    ...
    -- Retorna o centro bruto da UI
    return Vector2.new(pos.X + (size.X / 2), pos.Y + (size.Y / 2))
end

local function Click(x, y)
    -- Aplica o Inset (Mobile) + Seu Ajuste Personalizado
    local inset = S.GuiService:GetGuiInset()
    local finalX = x + inset.X + AjusteX
    local finalY = y + inset.Y + AjusteY
...
-- Busca Bruta pelo MiddleCircle (Lógica do primeiro script)
local function FindHookTarget()
    local hook = Player.PlayerGui:FindFirstChild("HookMeter")
    if hook then
        for _, v in ipairs(hook:GetDescendants()) do
            if v.Name == "MiddleCircle" or v.Name:find("Circle") then return v end
        end
    end
    return nil
end
...
local function StartBot()
    ...
    if NotificationService then
        NotificationService:Create("AutoFish", "🎯 Autofish Híbrido Ativado!")
    end
    ...
    task.spawn(function()
        while Bot.Enabled do
            if not Bot.Selling then
                ...
                if cur and max then
                    if NotificationService then
                        NotificationService:Update("AutoFish", ("Auto Fish Ativo! | %d / %d 🐟"):format(cur, max))
                    end
                    if cur >= max then DoAction("Sell") end
                end
            end
            task.wait(2)
        end
    ...
    Bot.Conn = S.RunService.Heartbeat:Connect(function()
        ...
        -- 1. LÓGICA DO PRIMEIRO MINI GAME (Hook/Fisgada)
        ...
        -- 2. LÓGICA DO SEGUNDO MINI GAME (Puxar/Catch)
        ...
        if img then
            ...
            if moving and target then
                ...
                local mC = GetCenter(moving)
                local tC = GetCenter(target)
                local delta = mC.X - tC.X

                -- Detecção de cruzamento com debounce de 0.1s
        ...
        elseif Bot.MinigameActive then
            ...
            task.wait(1.2)
            if Bot.Enabled and not Bot.Selling then DoAction("Throw") end
        end
    end)
end
...
PescaTab:Toggle("Autofish Full", false, function(state)
    ...
    if state then
        ...
        if NotificationService then NotificationService:Remove("AutoFish") end
        if Bot.Conn then Bot.Conn:Disconnect(); Bot.Conn = nil end
    end
end, { Keybind = { Value = ConfigPesca.AutoPescaKey } })
end
-- [TRECHO DO TELEPORT]
...
local function SetupChar(newChar)
    Char = newChar or LP.Character or LP.CharacterAdded:Wait()
...
local function createButton(name, text, pos, size)
    ...
    btn.TextScaled = true
    btn.Parent = flyControlsContainer
    
    local corner = Instance.new("UICorner")
    corner.CornerRadius = UDim.new(0, 8)
    corner.Parent = btn

    btn.MouseButton1Down:Connect(function() inputFlags[name:lower():gsub("button", "")] = true end)
    btn.MouseButton1Up:Connect(function() inputFlags[name:lower():gsub("button", "")] = false end)
    btn.MouseLeave:Connect(function() inputFlags[name:lower():gsub("button", "")] = false end)
    return btn
end

-- Botões de Direção (Esquerda)
local dpadSize = UDim2.new(0, 55, 0, 55)
createButton("Forward", "↑", UDim2.new(0, 75, 1, -180), dpadSize)
createButton("Back", "↓", UDim2.new(0, 75, 1, -60), dpadSize)
createButton("Left", "←", UDim2.new(0, 15, 1, -120), dpadSize)
createButton("Right", "→", UDim2.new(0, 135, 1, -120), dpadSize)

-- Botões de Altitude (Direita)
createButton("Up", "▲", UDim2.new(1, -70, 1, -180), dpadSize)
createButton("Down", "▼", UDim2.new(1, -70, 1, -60), dpadSize)
...
local function startFly()
    ...
    flyControlsContainer.Visible = true
end
...
local function stopFly()
    ...
    flyControlsContainer.Visible = false
    -- Limpa inputs
    for k in pairs(inputFlags) do inputFlags[k] = false end
end

RunService.RenderStepped:Connect(function()
    if not flying or not HRP or not Camera then return end

    local camCF = Camera.CFrame
    local moveDirection = Vector3.new(0, 0, 0)

    -- Inputs de Teclado (PC)
    if UserInputService:IsKeyDown(Enum.KeyCode.W) or inputFlags.forward then moveDirection += camCF.LookVector end
    if UserInputService:IsKeyDown(Enum.KeyCode.S) or inputFlags.back then moveDirection -= camCF.LookVector end
    if UserInputService:IsKeyDown(Enum.KeyCode.A) or inputFlags.left then moveDirection -= camCF.RightVector end
    if UserInputService:IsKeyDown(Enum.KeyCode.D) or inputFlags.right then moveDirection += camCF.RightVector end
    
    -- Vertical (Eixo Y puro ou baseado na Camera)
    if UserInputService:IsKeyDown(Enum.KeyCode.Space) or inputFlags.up then moveDirection += Vector3.new(0, 1, 0) end
    if UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or inputFlags.down then moveDirection -= Vector3.new(0, 1, 0) end
...
-- =========================
-- Nota: Certifique-se de que a variável 'Window' e 'ConfigExploit' existam no seu script principal
local ExploitTab = Window:Tab("Exploit", "rbxassetid://10734981995")

ExploitTab:Section("Movements")

ExploitTab:Toggle("Fly (Mobile Friendly)", false, function(state)
    if state then
        startFly()
    else
        stopFly()
