...
local function QuickMsg(title, text)
    if NotificationService then
        ...
        task.delay(0.8, function() NotificationService:Remove(id) end)
    end
end
...
local function StartBot()
    ...
    task.spawn(function()
        while Bot.Enabled do
            if not Bot.Selling then
                ...
                if cur and max and cur >= max then DoAction("Sell") end
            end
            task.wait(3)
        end
    end)
    ...
    Bot.Conn = S.RunService.Heartbeat:Connect(function()
        ...
        if mid and (os.clock() - Bot.LastHookClick >= Bot.HookCooldown) then
            ...
            local x, y = GetCorrectClickPos(mid)
            Click(x, y)
        end
        ...
        if img then
            ...
            if moving and target then
                ...
                local mX = moving.AbsolutePosition.X
