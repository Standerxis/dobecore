...
BV.MaxForce = Vector3.new(1e6, 1e6, 1e6) -- Força aumentada para evitar quedas
...
-- =========================
-- LÓGICA MOBILE / PC UNIFICADA
-- =========================
RunService.RenderStepped:Connect(function()
    if not flying or not HRP or not Hum then return end

    -- No Mobile, o MoveDirection pega o analógico perfeitamente
    local moveDir = Hum.MoveDirection 
    local camCF = Camera.CFrame
    
    if moveDir.Magnitude > 0 then
        -- Multiplicamos a direção do movimento pela velocidade
        -- Usamos a inclinação da câmera para voar na vertical (olhou pra cima, sobe)
        BV.Velocity = camCF:VectorToWorldSpace(camCF:PointToObjectSpace(camCF.Position + moveDir)) * SPEED
    else
        -- Mantém parado no ar se não estiver movendo o analógico
        BV.Velocity = Vector3.new(0, 0, 0)
    end

    -- Mantém o corpo alinhado com a câmera
    BG.CFrame = camCF
end)
...
local function startFly()
    ...
    SetupChar() -- Garante que as variáveis não estão nulas
...
-- =========================
-- EXEMPLO DE USO NA SUA TAB
-- =========================
-- Substitua 'SuaTab' pelo nome da sua aba na UI
PescaTab:Toggle("Fly Mobile", false, function(state)
    if state then
        startFly()
    else
        stopFly()
...
-- Função para converter o toque na tela em posição 3D no mapa
local function GetTouchPoint(touchPos)
    local ray = Camera:ViewportPointToRay(touchPos.X, touchPos.Y)
    local raycastParams = RaycastParams.new()
    raycastParams.FilterType = Enum.RaycastFilterType.Exclude
    raycastParams.FilterDescendantsInstances = {Player.Character} -- Não teleportar para o próprio corpo

    -- Lança um raio de 1000 studs de distância
...
-- Lógica unificada (PC e Mobile)
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed or not _G.ClickTPEnabled then return end
    
    local targetPos
    
    -- Se for Mobile (Toque na tela)
    if input.UserInputType == Enum.UserInputType.Touch then
        targetPos = GetTouchPoint(input.Position)
        
    -- Se for PC (Clique do Mouse)
    elseif input.UserInputType == Enum.UserInputType.MouseButton1 then
        local mouse = Player:GetMouse()
        targetPos = mouse.Hit.Position
    end

    -- Executa o teleporte se encontrar um ponto válido
    if targetPos then
        local character = Player.Character
        if character and character:FindFirstChild("HumanoidRootPart") then
            -- PivotTo é mais seguro que CFrame direto
