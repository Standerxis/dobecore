...
-- =========================
-- FUNÇÕES DE SUPORTE (Ajustadas para Mobile)
-- =========================
local function GetCenter(inst)
    if not inst then return Vector2.new(0,0) end
    -- AbsolutePosition já considera a escala da tela (resolução) automaticamente
    return Vector2.new(inst.AbsolutePosition.X + inst.AbsoluteSize.X / 2, inst.AbsolutePosition.Y + inst.AbsoluteSize.Y / 2)
end

local function Click(x, y)
    -- SendTapEvent não cria o cursor de mouse e clica exatamente na coordenada da UI
    -- Removido o GuiInset manual pois no Mobile o AbsolutePosition já é relativo à tela cheia
    ...
    Bot.Conn = S.RunService.Heartbeat:Connect(function()
        ...
        if img then
            ...
            if moving and target then
                ...
                -- MANTIDA SUA LÓGICA DE DELTA ORIGINAL
...
GraphicsTab:Toggle("Reshade Clean", false, function(state)
    if state then
        -- 1. BLOOM EQUILIBRADO (Glow sutil, sem estourar)
        local bloom = Instance.new("BloomEffect", Lighting)
        bloom.Intensity = 0.25   -- Reduzido drasticamente (era 0.8)
        bloom.Threshold = 0.85   -- Apenas luzes fortes brilham
        bloom.Size = 16          -- Brilho mais contido
        
        -- 2. COLOR CORRECTION (Cinematic Soft)
        local color = Instance.new("ColorCorrectionEffect", Lighting)
        color.Contrast = 0.2     -- Contraste suave
        color.Saturation = 0.25  -- Cores naturais mas vivas
        color.Brightness = 0.01  -- Quase neutro
        color.TintColor = Color3.fromRGB(255, 254, 250) -- Tom de papel limpo
        
        -- 3. DEPTH OF FIELD (Ajuste de Intensidade)
        local dof = Instance.new("DepthOfFieldEffect", Lighting)
        dof.FarIntensity = 0.4    -- Desfoque de fundo suave (era 0.9)
        dof.NearIntensity = 0.35  -- Desfoque de proximidade sutil (era 0.85)
        dof.FocusDistance = 15    
        dof.InFocusRadius = 35    -- Aumentado para ter uma área nítida maior
        
        shaderEffects = {bloom, color, dof}

        -- Loop de Processamento: Foco Dinâmico + Água
        connections.water = RunService.RenderStepped:Connect(function()
            local cam = workspace.CurrentCamera
            if not cam or not dof then return end
